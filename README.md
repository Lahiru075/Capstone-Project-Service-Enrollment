...
Enrollment-Service manages the full lifecycle of student-to-program enrollment records, backed by a MySQL relational database. It enriches each enrollment response with live student details by calling the Student-Service directly over REST. It registers itself with the Eureka Service-Registry and pulls its configuration from the Config-Server, making it a fully-managed member of the ECA microservices ecosystem.
...
│ • Validation & Mapping     │
└──────┬──────────────┬──────┘
       │              │ REST call (enrich response)
       ▼              ▼
┌──────────────┐  ┌───────────────────────┐
│ MySQL        │  │ Student-Service (8000)│
│ (Port 14500) │  └───────────────────────┘
└──────────────┘
       ▲
...
⚠️ **Prerequisites:** Config-Server, Service-Registry, Api-Gateway, and Student-Service must be running. A MySQL instance must be accessible on port 14500.
...
| 5️⃣ | Program-Service | 8001 |
| 6️⃣ | Enrollment-Service | 8002 |
